...
Relación entre la variable `estado_prestamo` y la calificación buró del cliente.  
...
### 1.2.1 Exploración de Visualizaciones  
#### 1.2.1.1 Ingreso Anual Atípico  
...
#### 1.2.1.2 Edad Atípica   
Atípico extremo en la clasicación **C**.   
```{r}
datos_prestamo %>%
    ggplot() +
    geom_point(aes(x = seq_along(edad), y = edad, color = factor(edad))) +
    labs(x = element_blank(), 
         y = "Edad") +
...
Se filtran:   
1. valores mayores que: **Q3 + 1.5 x RIQ**.  
2. Valores menores que: **Q1 - 1.5 x RIQ**
```{r}
datos_prestamo %>% count(ingreso_anual)
```
...
|Variable|Límites de Atípicos||Número de Obeservaciones Fuera de los Límites||
|---|---|---|---|---|
...
datos_prestamo_sinatp %>% 
    ggplot() +
    geom_histogram(aes(monto_prestado))
```
...
datos_prestamo_sinatp %>% 
    ggplot() +
    geom_histogram(aes(tasa_interes))
```
...
datos_prestamo_sinatp %>% 
    ggplot() +
    geom_boxplot(aes(x = seq_along(calificacion), y = calificacion))
```
...
datos_prestamo_sinatp %>% 
    ggplot() +
    geom_histogram(aes(antiguedad_trabajo))
```
...
datos_prestamo_sinatp %>% 
    ggplot() +
    geom_boxplot(aes(x = seq_along(propiedad_vivienda), y = propiedad_vivienda))
```
...
datos_prestamo_sinatp %>% 
    ggplot() +
    geom_histogram(aes(ingreso_anual))
```
...
Con `summary()` se puede detectar el número de NA's fácilmente.  
...
1. Eliminar las observaciones (*filas*), de cualquier parte de las variables que los contengan.  
...
2. Reemplazar los valores faltantes con la mediana de su respectiva variable.   

```{r reemplazo con la mediana}
median_tasa_interes <- median(datos_prestamo_sinatp$tasa_interes, na.rm = T)
median_antiguedad_trabajo <- median(datos_prestamo_sinatp$antiguedad_trabajo, na.rm = T)
datos_prestamo_sinatp_med <- datos_prestamo_sinatp
datos_prestamo_sinatp_med[is.na(datos_prestamo_sinatp_med$tasa_interes),"tasa_interes"] <- median_tasa_interes
datos_prestamo_sinatp_med[is.na(datos_prestamo_sinatp_med$antiguedad_trabajo),"antiguedad_trabajo"] <- median_antiguedad_trabajo
```

3. Mantener los valores faltantes de manera tal que el modelo pueda usarlos. Dado que ambas variables son continuas.  
...
Para evitar la pérdida de información potencial, se crean nuevas variables de categorización.   - `antiguedad_trabajo` -> `antiguedad_trabajo_cat` y    
 - `tasa_interes` -> `ti_cat`.  
